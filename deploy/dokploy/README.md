...
PGSSLROOTCERT_BASE64=<optional base64-encoded PEM root certificate from Supabase Connect>
...
`SUPABASE_DB_URL` is server-only. `PGSSLROOTCERT_BASE64` is the preferred single-line trusted TLS certificate setting for Dokploy; `PGSSLROOTCERT` is also accepted when the environment system preserves PEM newlines. Exactly one may be configured and both are used only with `rejectUnauthorized: true`. Never create a `VITE_SUPABASE_DB_URL`, never put a service-role/secret key in a `VITE_*` setting, and never paste real values into `.env.example`.
