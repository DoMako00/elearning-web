...
export function createTestAdminRequestContext(input: CreateTestAdminRequestContextInput): AdminRequestContext {
  ...
  const adminUserId = input.adminUserId ?? `${brandCode}-admin-001`;
  ...
  return { correlationId: input.correlationId ?? `${brandCode}-correlation-001`, brand, platform, subject: { providerSubjectId: input.providerSubjectId ?? `${brandCode}-provider-subject-001`, authProvider: "test", emailMasked: "admin@redacted.example" }, appUser: { appUserId, brandId, brandCode, platformId: brandId, platformCode: brandCode, status: "active" }, adminUser: { adminUserId, appUserId, brandId, brandCode, platformId: brandId, platformCode: brandCode, status: input.status ?? "active", roleIds: [`${brandCode}-role-001`], elevatedAccessExpiresAt: null }, session: { sessionId: `${brandCode}-session-001`, deviceId: `${brandCode}-device-001`, issuedAt: now, expiresAt: "2027-01-01T00:00:00.000Z" }, permissions: input.permissions ?? [] };
}