...
const courseScopes = new Set(["curriculum", "standalone"]);
...
function optionalText(correlationId: string, field: string, value: unknown): Result<void, AdminCoreError> { return value === undefined || value === null || (typeof value === "string" && value.trim().length > 0) ? ok(undefined) : invalid(correlationId, field); }
...
export function validateCreateInstructorCommand(command: CreateInstructorCommand): Result<void, AdminCoreError> { return first(command.metadata.correlationId, requiredText(command.metadata.correlationId, "displayName", command.displayName), optionalText(command.metadata.correlationId, "professionalTitle", command.professionalTitle)); }
export function validateUpdateInstructorCommand(command: UpdateInstructorCommand): Result<void, AdminCoreError> {
  ...
  if (command.displayName === undefined && command.professionalTitle === undefined) return invalid(correlationId, "update");
  return first(correlationId, requiredUuid(correlationId, "instructorId", command.instructorId), command.displayName === undefined ? ok(undefined) : requiredText(correlationId, "displayName", command.displayName), optionalText(correlationId, "professionalTitle", command.professionalTitle));
}
export function validateSetInstructorStatusCommand(command: SetInstructorStatusCommand): Result<void, AdminCoreError> { const id = command.metadata.correlationId; return first(id, requiredUuid(id, "instructorId", command.instructorId), status(id, "status", command.status, activeInactive)); }
...
export function validateBrandCourseDefinition(correlationId: string, courseScope: unknown, academicModuleId: unknown): Result<void, AdminCoreError> {
  ...
  return courseScope === "curriculum" && !academicModuleId ? invalid(correlationId, "academicModuleId") : ok(undefined);
}
...
export function validateUpdateBrandCourseCommand(command: UpdateBrandCourseCommand): Result<void, AdminCoreError> {
  ...
  if (command.courseScope === "curriculum" && command.academicModuleId === null) return invalid(id, "academicModuleId");
