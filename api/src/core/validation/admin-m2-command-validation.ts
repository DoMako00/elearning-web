...
export function validateCreateBrandCourseCommand(command: CreateBrandCourseCommand): Result<void, AdminCoreError> { const id = command.metadata.correlationId; return first(id, requiredUuid(id, "brandId", command.brandId), requiredUuid(id, "academicInstitutionId", command.academicInstitutionId), requiredText(id, "courseCode", command.courseCode), requiredText(id, "title", command.title), validateBrandCourseDefinition(id, command.courseScope, command.academicModuleId), command.cataloguePresentation === undefined ? ok(undefined) : status(id, "cataloguePresentation", command.cataloguePresentation, cataloguePresentations), status(id, "status", command.status, new Set(["draft"]))); }
export function validateUpdateBrandCourseCommand(command: UpdateBrandCourseCommand): Result<void, AdminCoreError> {
  ...
  if (command.title === undefined && command.courseScope === undefined && command.academicModuleId === undefined && command.academicInstitutionId === undefined && command.cataloguePresentation === undefined) return invalid(id, "update");
  if (command.courseScope === "academic_module_offering" && command.academicModuleId === null) return invalid(id, "academicModuleId");
  return first(id, requiredUuid(id, "brandId", command.brandId), requiredUuid(id, "courseId", command.courseId), command.academicInstitutionId === undefined ? ok(undefined) : requiredUuid(id, "academicInstitutionId", command.academicInstitutionId), command.title === undefined ? ok(undefined) : requiredText(id, "title", command.title), command.courseScope === undefined ? ok(undefined) : status(id, "courseScope", command.courseScope, courseScopes), command.academicModuleId === undefined || command.academicModuleId === null ? ok(undefined) : requiredUuid(id, "academicModuleId", command.academicModuleId), command.cataloguePresentation === undefined ? ok(undefined) : status(id, "cataloguePresentation", command.cataloguePresentation, cataloguePresentations));
}
