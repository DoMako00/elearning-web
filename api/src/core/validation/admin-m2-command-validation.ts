...
export function validateUpdateBrandCourseCommand(command: UpdateBrandCourseCommand): Result<void, AdminCoreError> {
  ...
  if (command.title === undefined && command.courseScope === undefined && command.academicModuleId === undefined && command.academicInstitutionId === undefined) return invalid(id, "update");
