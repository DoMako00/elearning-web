...
export interface M2CourseInstructorAssignmentFacts extends M2BrandInstructorAssignmentFacts { readonly courseExistsInBrand: boolean; readonly courseStatus?: M2BrandCourseStatus; readonly brandInstructorStatus?: M2InstructorStatus; readonly courseInstructorStatus?: M2InstructorStatus; }
...
export function validateM2BrandCourseDefinitionPolicy(correlationId: string, snapshot: M2BrandCourseDefinitionSnapshot): Result<void, AdminCoreError> {
  if (snapshot.courseScope === "academic_module_offering" && !snapshot.academicModuleId) return rejected(correlationId, "academic_module_required");
...
export function validateAssignInstructorToCoursePolicy(correlationId: string, facts: M2CourseInstructorAssignmentFacts): Result<{ readonly idempotent: boolean }, AdminCoreError> {
  ...
  if (facts.courseStatus === "archived") return rejected(correlationId, "course_archived");
