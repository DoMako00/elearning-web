...
export class InMemoryAdminEvidenceWriter implements AdminEvidenceWriter {
  ...
  private sequence = 0;
  private write(input: { context: AdminRequestContext; targetBrandId?: string; targetPlatformId?: string }, prefix: string, data: Readonly<Record<string, unknown>>): Result<AdminEvidenceWriteResult, AdminCoreError> { if (input.context.brand.brandId !== (input.targetBrandId ?? input.targetPlatformId)) return fail(targetPlatformMismatchError(input.context.correlationId)); this.sequence += 1; const result = { id: `${prefix}-${String(this.sequence).padStart(4, "0")}`, correlationId: input.context.correlationId, writtenAt: new Date().toISOString() }; const record = { ...result, ...data }; if (prefix.startsWith("audit")) this.auditLogs.push(record); else if (prefix.startsWith("admin-action")) this.adminActions.push(record); else this.securityEvents.push(record); return ok(result); }
