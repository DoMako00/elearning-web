...
import { validateAdminTargetPlatform, validateCommandPlatform, validateSensitiveCommandMetadata } from "../../core/validation";
import { adminCoreError, type AdminCoreError } from "../../core/errors";
import type { AdminEvidenceWriter } from "../../core/logging";

export interface AdminCommandTarget { readonly targetType: string; readonly targetId: string; readonly targetPlatformId: string; }
...
export async function executeAdminCommandBoundary<T>(input: AdminCommandBoundaryInput<T>, dependencies: AdminCommandBoundaryDependencies): Promise<AdminCommandBoundaryResult<T>> {
  ...
  const commandPlatform = validateCommandPlatform(input.metadata, input.context); if (!commandPlatform.ok) return deny<T>(input, dependencies, commandPlatform.error);
  ...
  const targetPlatform = validateAdminTargetPlatform(effectiveContext, input.target.targetPlatformId); if (!targetPlatform.ok) return deny<T>(input, dependencies, targetPlatform.error);
  const policy = await dependencies.policyValidator.validateCommandPolicy({ context: effectiveContext, commandName: input.commandName, targetType: input.target.targetType, targetId: input.target.targetId, targetPlatformId: input.target.targetPlatformId, policySetId: input.metadata.policySetId, expectedVersion: input.metadata.expectedVersion }); if (!policy.ok) return deny<T>(input, dependencies, policy.error);
  const result = await input.run(); if (!result.ok) { await writeDeniedOrFailed(input, dependencies, "failed"); return fail(result.error); }
  const action = await dependencies.evidenceWriter.writeAdminAction({ context: effectiveContext, commandName: input.commandName, targetType: input.target.targetType, targetId: input.target.targetId, targetPlatformId: input.target.targetPlatformId, outcome: "succeeded", reason: input.metadata.reason }); if (!action.ok) return fail(adminCoreError("audit_write_failed", "The administrative evidence could not be written.", input.metadata.correlationId));
  let auditEventId: string | undefined;
  if (input.writeAudit) { const audit = await dependencies.evidenceWriter.writeAuditLog({ context: effectiveContext, action: input.commandName, targetType: input.target.targetType, targetId: input.target.targetId, targetPlatformId: input.target.targetPlatformId, reason: input.metadata.reason }); if (!audit.ok) return fail(adminCoreError("audit_write_failed", "The administrative evidence could not be written.", input.metadata.correlationId)); auditEventId = audit.value.id; }
