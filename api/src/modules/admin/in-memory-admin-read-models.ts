...
export interface AdminOverviewReadModel { readonly brand: AdminBrandContext; /** @deprecated Compatibility projection for existing consumers. */ readonly platform: AdminPlatformContext; readonly counts: AdminOverviewCounts; readonly recent: { readonly auditLogs: readonly AdminAuditLogItem[]; readonly adminActions: readonly AdminAdminActionItem[]; readonly securityEvents: readonly AdminSecurityEventItem[] }; }
export interface AdminReadModels { getOverview(brandId: string): AdminOverviewReadModel | undefined; }
export class InMemoryAdminReadModels implements AdminReadModels { constructor(private readonly overviewByBrand: ReadonlyMap<string, AdminOverviewReadModel>) {} getOverview(brandId: string) { return this.overviewByBrand.get(brandId); } }
