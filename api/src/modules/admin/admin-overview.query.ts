...
export async function getAdminOverview(context: AdminRequestContext, readModel: AdminOverviewReadModel): Promise<Result<AdminOverviewResult, AdminCoreError>> {
  ...
  const modelResult = await readModel.getOverview({ brandCode: context.brand.brandCode, correlationId: context.correlationId });
  if (!modelResult.ok) {
    const code = modelResult.error.code === "brand_not_found" || modelResult.error.code === "not_found"
      ? "target_brand_mismatch"
      : "validation_failed";
    return fail(adminCoreError(code, modelResult.error.code === "brand_not_found" || modelResult.error.code === "not_found"
      ? "No admin overview is available for the resolved brand."
      : "Admin overview data is unavailable.", context.correlationId));
  }
  const model = modelResult.value;
  if (model.brand.brandCode !== context.brand.brandCode) return fail(adminCoreError("target_brand_mismatch", "No admin overview is available for the resolved brand.", context.correlationId));
