...
export interface AdminModuleDependencies { readonly permissionResolver: AdminPermissionResolver; readonly policyValidator: AdminPolicyValidator; readonly evidenceWriter: AdminEvidenceWriter; readonly overviewReadModel?: AdminOverviewReadModel; readonly m2ReadModel?: AdminM2ReadModel; }
export function createAdminModule(dependencies: AdminModuleDependencies) {
  ...
  return { queries: { getAdminOverview: (context: AdminRequestContext) => getAdminOverview(context, overviewReadModel), m2: m2ReadModel }, commands: { executeAdminCommandBoundary: <T>(input: AdminCommandBoundaryInput<T>) => executeAdminCommandBoundary(input, boundary) } };
}
