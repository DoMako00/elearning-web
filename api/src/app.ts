...
import { createAdminModule, createAdminOverviewReadModel, createAdminM2ReadModel, createAdminM2CommandRuntime, resolveAdminOverviewReadModelSource, resolveAdminM2ReadModelSource, resolveAdminCommandSource, type AdminModuleDependencies, type AdminOverviewReadModelSource, type AdminM2ReadModelSource, type AdminCommandSource } from "./modules/admin";
...
export function createApplication(options: BackendApplicationOptions = {}): BackendApplication {
  ...
  return { status: "configured-admin-core-boundary", admin: createAdminModule(adminDependencies), adminDependencies, persistence, adminOverviewSource, adminM2Source, adminCommandSource, close: () => { if (!closePromise) closePromise = Promise.all([commandRuntime.close(), persistence.close()]).then(() => undefined); return closePromise; } };
}
