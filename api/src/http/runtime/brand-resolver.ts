...
export function resolveBrandFromRequestUrl(url: string, correlationId: string): Result<AdminBrandCode, BrandResolutionError> { const value = new URL(url, "http://localhost").searchParams.get("brand"); if (!value) return fail({ code: "brand_required", message: "A brand query parameter is required.", correlationId }); if (value !== "medway" && value !== "elite" && value !== "nexus") return fail({ code: "brand_invalid", message: "The requested brand is not supported.", correlationId }); return ok(value); }
