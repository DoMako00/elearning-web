...
for(const entity of ['chapters','lessons','resources'] as const){
  ...
  const headers=[{name:'Idempotency-Key',in:'header',required:true,schema:{type:'string',maxLength:128,pattern:'^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$'},description:'Required only for POST/PATCH mutation operations. Do not send on GET list operations.'}];
  ...
  const write=(operationId:string,schema:unknown)=>({tags:['Course delivery'],operationId,security,description,requestBody:{required:true,content:json(schema)},responses:{...errors,'200':{description:'Applied, unchanged, or replayed command',content:json(commandResult)},'201':{description:'Created metadata',content:json(commandResult)}}});
  m2bPaths[base]={get:{tags:['Course delivery'],operationId:'list'+name+'s',security,description:description+' This is a GET read operation and never requires Idempotency-Key.',summary:'List '+entity,parameters,responses:{...errors,'200':{description:'Real course structure; empty array when no records exist',content:json(list)}}},post:{...write('create'+name,create),summary:'Create '+name,parameters:[...parameters,...headers]}};
  m2bPaths[base+'/{'+entityId+'}']={patch:{...write('update'+name,update),summary:'Update '+name,parameters:[...parameters,pathId(entityId),...headers]}};
}
