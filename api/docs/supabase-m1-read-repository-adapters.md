...
Prompt 41 groups these adapters into a typed runtime bundle behind `PERSISTENCE_PROVIDER=supabase`; the bundle is constructed without querying and remains unused by Admin Overview until Prompt 42.

Prompt 42 uses only the educational-brand M1 read contract for the first PostgreSQL Admin Overview source. Current overview cards require later domains, so no membership, profile, global-user, subscription, payment, or access metric is inferred from M1.
## Prompt 39 transport
