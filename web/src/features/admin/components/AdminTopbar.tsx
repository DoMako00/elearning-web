...
export function AdminTopbar({ brand, brandView, availableBrands, setBrandView }: { brand?: AdminBrandContext; brandView: AdminBrandView; availableBrands: readonly AdminBrandContext[]; setBrandView: (view: AdminBrandView) => void }) {
  ...
  return <header className={`admin-topbar${pathname === "/admin/instructors" ? " is-instructors" : ""}`}>
    ...
    <div className="admin-topbar__controls">
      {isCurriculum ? <div className="admin-curriculum-scope" aria-label="Academic catalogue scope"><BookOpen aria-hidden="true" /><span>Academic Catalogues</span></div> : <div className="admin-brand-selector" aria-label="Admin brand viewing context">{views.map((item, index) => <button key={item.code} type="button" aria-label={item.code === "all" ? "View all brands" : `View ${item.label}`} aria-pressed={brandView === item.code} className={item.code === "all" ? "is-all" : item.code === "elite" ? "is-elite" : "is-medway"} onKeyDown={(event) => moveView(event, index)} onClick={() => setBrandView(item.code)}>{item.code === "all" ? <Layers3 aria-hidden="true" /> : item.code === "elite" ? <Crown aria-hidden="true" /> : <ShieldCheck aria-hidden="true" />}<span data-short={item.shortLabel}>{item.label}</span></button>)}</div>}
      <button className="admin-icon-button" type="button" aria-label="Open notifications preview" onClick={() => setNotice("Notifications are preview-only in this milestone.")}><Bell aria-hidden="true" /></button>
