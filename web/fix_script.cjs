...
const content = fs.readFileSync('C:\\Users\\shehab\\OneDrive\\Desktop\\E-learning stage\\elearning-web\\web\\src\\components\\ui\\InactivityPrompt\\InactivityModal.tsx', 'utf8');
...
if (insertIdx !== -1) {
  ...
  const insertPos = content.indexOf(searchStr) + searchStr.length;
  const newContent = content.slice(0, insertIdx + searchStr.length) + codeToInsert + content.slice(insertIdx + searchStr.length);
  fs.writeFileSync('C:\\Users\\shehab\\OneDrive\\Desktop\\E-learning stage\\elearning-web\\web\\src\\components\\ui\\InactivityPrompt\\InactivityModal.tsx', newContent, 'utf8');
